...
def myamtl_params():
    mu = np.linspace(0.001, 10, 10)
    lamb = np.linspace(0.001, 10, 10)
    res = list()
    for m1 in mu:
        for l1 in lamb:
...
def glasso_params():
    lambda_1 = np.linspace(0.001, 15, 2)
    res = list()
    for l1 in lambda_1:
        res.append({'lambda_1': l1})
    return res
...
def gamtl_params():
    lambda_1 = np.linspace(0.001, 3, 2)
    lambda_2 = np.linspace(0.001, 0.5, 2)
    lambda_3 = np.linspace(0.005, 0.1, 2)
    res = list()
    for l1 in lambda_1:
        for l2 in lambda_2:
            for l3 in lambda_3:
...
def myamtl_params():
    mu = np.linspace(0.001, 10, 2)
    lamb = np.linspace(0.001, 10, 2)
    res = list()
    for m1 in mu:
        for l1 in lamb:
            res.append({'mu': m1, 'lamb': l1})
...
def myamtl2_params():
    lambda_1 = np.linspace(0.001, 1, 2)
    lambda_2 = np.linspace(0.01, 1, 10)
    lambda_3 = [0]
    res = list()
    for l1 in lambda_1:
...
if __name__ == "__main__":
    ...
    # strategies.add(MyAMTL, {}, myamtl_params())
    groups = np.array([[0],
                       [50],
                       [np.sqrt(50)]])
